...
class MultKAN(nn.Module):
    ...
    def __init__(self, width=None, grid=3, k=3, mult_arity=2, noise_scale=0.3, scale_base_mu=0.0, scale_base_sigma=1.0,
                 ...
                 sp_trainable=True, sb_trainable=True, seed=1024, save_act=True, sparse_init=False, auto_save=True,
    ...
    def plot(self, file_name, folder="./figures", beta=3, metric='backward', scale=0.5, tick=False, sample=False,
             in_vars=None, out_vars=None, title=None, varscale=1.0):
        '''
        plot KAN
            ...
            for i in range(self.width_in[l]):
                for j in range(self.width_out[l + 1]):
                    ...
                    if alpha_mask == 1:
                        ...
                        # plt.gca().patch.set_edgecolor('#a63539')
                    # a63539
                    else:
                ...
                if l < neuron_depth - 1:
                    ...
                    for j in range(n_next):
                        ...
                        plt.plot([1 / (2 * n) + i / n, 1 / (2 * N) + id_ / N],
                                 [l * (y0 + z0), l * (y0 + z0) + y0 / 2 - y1], color=color, lw=4 * scale,
                                 alpha=alpha[l][j][i] * alpha_mask)
                        plt.plot([1 / (2 * N) + id_ / N, 1 / (2 * n_next) + j / n_next],
                                 [l * (y0 + z0) + y0 / 2 + y1, l * (y0 + z0) + y0], color=color, lw=4 * scale,
                                 alpha=alpha[l][j][i] * alpha_mask)
            ...
            if l < neuron_depth - 1:
                ...
                for i in range(n_in):
                    ...
                    plt.plot([1 / (2 * n_in) + i / n_in, 1 / (2 * n_out) + j / n_out],
                             [l * (y0 + z0) + y0, (l + 1) * (y0 + z0)], color='black', lw=4 * scale)
        ...
        #     counter += 1
        if not os.path.exists("kan_plot"):
            os.makedirs("kan_plot")

        if file_name.split('.')[-1] != 'png':
            file_name = file_name + ".png"
        file_path = "./kan_plot/" + file_name
